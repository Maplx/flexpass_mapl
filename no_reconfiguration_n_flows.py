...
n_flows_values = [4,6,8,10,12,14,16,18,20] # Varying number of applications
T = 50
links = range(30)
max_n_states = 10
max_n_flows = 8
...
def run_simulation(AdjustmentClass, n_flows, trial):
    inf_l = []
    sr_l = []
    flex_l = []
    par_l = []
    ...
    while i < trial:
        adj = AdjustmentClass(k, n_apps=5, T=T, links=links,
                          max_n_states=10, max_n_flows=n_flows,
...
for n_apps in tqdm(n_flows_values, desc="Running simulations for different n_apps"):
...
def plot_metric(metric, label):
    plt.figure()
    plt.plot(n_flows_values, [res[metric] for res in results_heu], label="Heuristic ", marker='o')
    plt.plot(n_flows_values, [res[metric] for res in results_adjust1], label="Guaranteed-Only  ", marker='o')
    plt.plot(n_flows_values, [res[metric] for res in results_setbip], label="SetCover+Bipartite", marker='o')
    plt.plot(n_flows_values, [res[metric] for res in results_bipartite], label="Conditional", marker='o')

    plt.xlabel("Number of Flows (n_flows)")
    plt.ylabel(label)
    plt.xticks(n_flows_values)  # Set x-axis ticks to be exactly the n_apps values
    plt.title(f"{label} vs. Number of Flows")
    plt.legend()
...
'''
[{'sr': 0.995, 'avg_flex': 4.819320552763819, 'adjusted partitions': 5.0}, {'sr': 0.995, 'avg_flex': 4.519324924623113, 'adjusted partitions': 5.0}, {'sr': 0.98, 'avg_flex': 3.9637367346938803, 'adjusted partitions': 5.0}, {'sr': 0.985, 'avg_flex': 3.414887918781728, 'adjusted partitions': 5.0}, {'sr': 0.965, 'avg_flex': 3.0112312435233166, 'adjusted partitions': 5.0}, {'sr': 0.905, 'avg_flex': 2.503649392265192, 'adjusted partitions': 5.0}, {'sr': 0.895, 'avg_flex': 2.2533450837988838, 'adjusted partitions': 5.0}, {'sr': 0.825, 'avg_flex': 2.061200060606061, 'adjusted partitions': 5.0}, {'sr': 0.815, 'avg_flex': 2.0406972392638045, 'adjusted partitions': 5.0}]
[{'sr': 0.225, 'avg_flex': 3.684442320083449, 'adjusted partitions': 1.72}, {'sr': 0.165, 'avg_flex': 3.3766665053275906, 'adjusted partitions': 1.8461538461538463}, {'sr': 0.18, 'avg_flex': 2.9533118007653707, 'adjusted partitions': 1.8125}, {'sr': 0.165, 'avg_flex': 2.722650528970736, 'adjusted partitions': 1.6923076923076923}, {'sr': 0.18, 'avg_flex': 1.898256770638958, 'adjusted partitions': 2.5}, {'sr': 0.175, 'avg_flex': 1.3170981197641218, 'adjusted partitions': 2.8666666666666667}, {'sr': 0.18, 'avg_flex': 1.4284609201935028, 'adjusted partitions': 2.625}, {'sr': 0.165, 'avg_flex': 1.3214869769232227, 'adjusted partitions': 2.076923076923077}, {'sr': 0.195, 'avg_flex': 1.6575648624893236, 'adjusted partitions': 2.263157894736842}]
